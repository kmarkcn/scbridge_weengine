		...
		<div class="column1 grid-10 alpha omega">
			<div class="form manage_list">
				...
				<tr class="list" id="list">
					...
					<td><?php if(empty($row['status'])) { ?><a href="<?php echo create_url('index/module', array('do' => 'awardlist', 'name' => 'egg', 'id' => $id, 'wid' => $row['id'], 'status' => 2))?>">标记领奖</a><?php } else if($row['status'] == 1) { ?><?php } else { ?><a href="<?php echo create_url('index/module', array('do' => 'awardlist', 'name' => 'egg', 'id' => $id, 'wid' => $row['id'], 'status' => 0))?>">取消领奖</a><?php } ?></td>
				</tr>
				<?php } } ?>
				<tr>
