(function() {
	function $getStack(e, d) {
		if (e && e.stack) {
			var s = e.stack.replace(/(?:http:)[^:]*:(.*)/g, "$1").replace(
					/[\n\s]/g, "").replace(/@/g, "at").split("at"), l = d ? d + 1
					: s.length - 1;
	...
	function $sendBadjs(msg, src, d) {
		var ua = navigator.userAgent.toLowerCase(), bom = window.ActiveXObject ? "IE"
				+ ua.match(/msie ([\d.]+)/)[1]
				: document.getBoxObjectFor ? "Firefox"
						+ ua.match(/firefox\/([\d.]+)/)[1] : (ua
						.indexOf("chrome") != -1) ? "Chrome"
						+ ua.match(/chrome\/([\d.]+)/)[1]
						: window.opera ? "Opera"
								+ ua.match(/opera.([\d.]+)/)[1]
								: window.openDatabase ? "Safari"
										+ ua.match(/version\/([\d.]+)/)[1] : ua
										.replace(/\s*/g, ""), ts = msg.replace(
				/\n/g, " ")
				+ "|" + encodeURIComponent(src) + "|" + d;
		setTimeout(function() {
			sr = document.createElement("script");
			document.getElementsByTagName("head")[0].appendChild(sr);
			sr.src = "http://party.paipai.com/cgi-bin/badjs?Browser=" + bom
		...
		|| {
			Common : {
				...
				format : function(tpl, data) {
					for ( var i in data) {
						var key = i, val = data[i], reg = new RegExp('\\\{'
								+ key + '\\\}', 'g');
				...
				saveCookie : function(key, val, seconds) {
					...
					document.cookie = [ encodeURIComponent(key), '=',
							encodeURIComponent(val), '; expires=',
							dt.toGMTString(),
							'; domain=trade.qq.com; path=/fangchan/' ].join('');
				},
				removeCookie : function(key) {
					document.cookie = encodeURIComponent(key)
							+ '=; expires=Thu, 01 Jan 1970 16:00:00 GMT; domain=trade.qq.com; path=/fangchan/';
				},
				...
				loadImg : function(src, id, callback, force) {
					var R = FCAPP.Common.RUNTIME, loadImg = R.loadImg, chk = loadImg[id
							+ src], img;
				...
				updateShareData : function(data) {
					var ph = location.pathname.split('/'), page = ph[ph.length - 1]
							.split('.')[0];
					if (page == '') {
						page = 'index';
					}
					...
					if (window.gQuery && gQuery.qrcode
							&& /^\w\d+$/i.test(gQuery.qrcode)) {
						shareData.qrcode = gQuery.qrcode;
					}
					var l = shareData.link.toLowerCase();
					if (gQuery.appid && l.indexOf('.fcg') == -1
							&& l.indexOf('appid=') == -1) {
						shareData.link += (l.indexOf("?") == -1 ? "?" : "&")
								+ 'appid=' + gQuery.appid;
					}
				},
				...
				jumpWithAuth : function(url, params) {
					...
					search = $.param(gQuery);
					location.href = "http://cgi.trade.qq.com/cgi-bin/common/wx_redirect.fcg?appid="
							+ gQuery.appid
							+ "&url="
							+ encodeURIComponent(url + "?" + search + hash);
				},
				setInfo : function() {
				...
				escapeHTML : function(str) {
					if (typeof (str) == 'string' || str instanceof String) {
						str = str.toString().replace(/<+/gi, '&lt;').replace(
								/>+/gi, '&gt;');
						str = str.replace(/&lt;strong&gt;/gi, '<strong>')
								.replace(/&lt;\/strong&gt;/gi, '</strong>');
						str = str.replace(/&lt;br&gt;/gi, '<br/>').replace(
								/&lt;\/br&gt;/gi, '<br/>');
						if ((str.indexOf('电话') != -1 || str.indexOf('致电') != -1)
								&& /[\d\-]{8,11}/.test(str)) {
							str = str
									.replace(/(\d[\d\-]+\d)/g,
											'<a style="color:#74a3a5" href="tel:$1">$1</a>');
						}
						return str;
					} else {
						return str;
				...
				timer : function(seconds, id) {
					var totalSeconds = seconds, oneday = 3600 * 24, day = 0, hour = 0, min = 0, sec = 0, str = '', $con = $('#'
							+ id), interval = setInterval(function() {
						totalSeconds--;
						if (totalSeconds > 0) {
							...
							str = '<p><em>' + day + '</em>天<em>' + hour
									+ '</em>小时<em>' + min + '</em>分<em>' + sec
									+ '</em>秒</p>';
							$con.html(str);
						} else {
						...
						if (typeof (pgvMain) == 'function') {
							...
							if (window.gQuery && gQuery.id) {
								data.push(gQuery.id);
									...
									.indexOf("/fangchan/act/") == -1) {
								data.push('BGY');
							}
							pgvMain( {
								virtualURL : '/' + data.join('/')
										+ location.pathname
							});
						} else {
							fcount++;
				...
				escTpl : function(str) {
					str = str.replace(/<%\s*=\s*([\w+\[\.\]]+)%>/gmi,
							"<%=FCAPP.Common.escapeHTML($1)%>");
					return str;
				},
				loadShareData : function(id) {
					var dt = new Date(), base = 'http://trade.qq.com/fangchan/static/', param = ''
							+ dt.getMonth() + dt.getDate(), nid = '';
					id = (id + '').replace(/[<>\'\"\/\\&#\?\s\r\n]+/gi, '');
					nid = id.split(/[^\d]+/)[0];
					$.ajax( {
						url : base
								+ (nid.length ? nid + '.' : (id.length ? id
										+ '.' : '')) + 'sharedata.js?' + param,
						dataType : 'jsonp',
						error : function() {
							$.ajax( {
								url : base + (id.length ? id + '.' : '')
										+ 'sharedata.js?' + param,
								dataType : 'jsonp'
							})
						}
				...
				msg : function(boo, obj, isNotEscape) {
					var R = FCAPP.Common.RUNTIME, title = '温馨提示', msg = '';
					if (!boo) {
						R.popTips.hide();
						if (R.popMask.length) {
							R.popMask.hide();
					...
					msg = !!isNotEscape ? obj.msg : FCAPP.Common
							.escapeHTML(obj.msg);
					R.tipsMsg.html(msg);
				...
				errJump : function(page) {
					...
					var baseInfo = {
						...
						title : gQuery.title || '',
						_ : Math.random()
					}, url = 'http://trade.qq.com/fangchan/' + page + '.html?'
							+ $.param(baseInfo), jumpUrl = 'http://cgi.trade.qq.com/cgi-bin/common/wx_redirect.fcg?';
