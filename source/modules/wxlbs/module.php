...
class WxlbsModule extends WeModule {
	public $name = 'WxlbsModule';
	public $title = '地理位置服务';
	public $ability = '';
	public $tablename = 'wxlbs';
...
public function fieldsFormDisplay($rid = 0) {
	...
	public function dolocationDetails() {
		global $_W, $_GPC;
		...
		$sql = "SELECT * FROM " . tablename('wxlbs') . " WHERE `id`=:id LIMIT 1";
		$location = pdo_fetch($sql, array(':id' => $id));
		
		$directionPath = $_W['siteroot'] . 'source/modules/wxlbs/baidumapDirection.php?t1='.$t1.'&t2='.$t2;
		$location['picture'] = empty($location['picture']) ? 'http://api.map.baidu.com/staticimage?center='.$lng.','.$lat.'&markers='.$lng.','.$lat.'&zoom=14&dpiType=ph' : $_W['attachurl'].$location['picture'];
		if (empty($location['id'])) {
			echo '您选择的地址不存在，请联系管理员';
		}
		
		include $this->template('wxlbs:locationDetails');
